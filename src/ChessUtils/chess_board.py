...
class board():
    ...
    def coord_to_square(self, i, j):
        """Convertit des indices (i:0–7, j:0–7) en case algébrique, ex. (7,4) → 'e1'."""
        file = chr(ord('a') + j)
        rank = str(8 - i)
        return file + rank
    
    def get_move_details(self, before: np.ndarray, after: np.ndarray):
        """
        Compare two board states and return move details:
        - from_sq   : (moving_piece, origin_square)
        - to_sq     : (moved_or_captured_piece, destination_square)
        - move_type : 'O-O', 'O-O-O', capture square for ordinary/en-passant, or None
        """
        # 1) List all differences
        diffs = [(i, j) for i in range(8) for j in range(8)
                if before[i, j] != after[i, j]]

        # 2) Castling shortcuts
        if before[7,4]=='K' and after[7,6]=='K': return ('K','e1'), ('K','g1'), 'O-O'
        if before[7,4]=='K' and after[7,2]=='K': return ('K','e1'), ('K','c1'), 'O-O-O'
        if before[0,4]=='k' and after[0,6]=='k': return ('k','e8'), ('k','g8'), 'o-o'
        if before[0,4]=='k' and after[0,2]=='k': return ('k','e8'), ('k','c8'), 'o-o-o'

        # 3) Find destination square (where a new piece appears)
        dest = next(((i, j) for i, j in diffs
                    if after[i, j] != ' ' and before[i, j] != after[i, j]), None)
        if dest is None:
            raise ValueError("Impossible de trouver la destination.")
        i_dest, j_dest = dest
        dest_str = self.coord_to_square(i_dest, j_dest)
        moving_piece = after[dest]

        # 4) Find origin square (where the moving piece disappeared)
        vacated = [(i, j) for i, j in diffs if before[i, j] != ' ' and after[i, j] == ' ']
        origin = next(((i, j) for i, j in vacated if before[i, j] == moving_piece), None)
        if origin is None:
            raise ValueError("Impossible de trouver l'origine du coup.")
        from_str = self.coord_to_square(*origin)

        # 5) Ordinary capture: if the destination held an enemy piece before the move
        if before[dest] != ' ':
            cap_piece = before[dest]
            # move_type is the capture square in algebraic
            move_type = dest_str
            from_sq = (moving_piece, from_str)
            to_sq = (cap_piece, dest_str)
            return from_sq, to_sq, move_type

        # 6) En-passant or multi-square captures: detect second vacated square
        capture_sq = next((sq for sq in vacated if sq != origin), None)
        if capture_sq:
            cap_piece = before[capture_sq]
            cap_str = self.coord_to_square(*capture_sq)
            from_sq = (moving_piece, from_str)
            to_sq = (cap_piece, dest_str)
            move_type = cap_str
            return from_sq, to_sq, move_type
